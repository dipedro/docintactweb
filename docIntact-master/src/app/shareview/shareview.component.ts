...
this.iebrowser=false
  ...
  displayUpdatedFieldValue(FieldValue){
    if(this.fields.length)
      ...
      if(this.profiledata && (FieldValue.insertedemail!==this.profiledata.email)) this.dependencyCheck(this.fields)
  }
  ...
  documentStatusUpdate(fields) {
    return new Promise(async (resolve, reject) => {
      ...
      this.documentService.getsharingpeople(this.sharedRecord.fileid._id).subscribe((sharerecords: any) => {
        ...
        // if (this.sharedRecord.view) {
        //   if (reviewedPeople.length == sharerecords.length)
        //     status = "Completed"
        //   else if (signedPeople.length || reviewedPeople.length)
        //     status = "Partially completed"
        // }
        // else if (!this.sharedRecord.view) {
          //  if (((statusemail.length == shareForSignatures.length) || (filledFieldCount == fields.length)) && shareAgree.length == sharerecords.length)
          //   status = "Completed"
          //   else if (sharerecords.length >= 1 && (statusemail.length >= 1 || filledFieldCount > 1))
          //   status = "Partially completed"

          if(sharerecords.length && ((signedPeople.length==shareForSignatures.length && reviewedPeople.length==shareForReviews.length) || (reviewedPeople.length==shareForReviews.length && filledFieldCount == fields.length)))
            status = "Completed"
        ...
        dialogRef.afterClosed().subscribe(res => {
          if (res) {
            ...
            this.documentService.getCurrentVersionDocFieldWithValues({ documentid: this.documentRecord._id, versionid: this.documentRecord.versionid }).subscribe(async currentVersionDocFieldOptions => {
              ...
              // if(this.sharedRecord.view) this.sharedRecord.reviewed = true
              // else this.sharedRecord.signed = true
...
focusfield(item)
{
  ...
  savedocLogs(fileid, fielddata, message) {
    ...
    if (message == 'Closed' || message == 'Submited' || message == 'Reviewed') {
  ...
  review() {
    if (!this.sharedRecord.reviewed) {
      let dialogdata = this.dialog.open(CommonDialogComponent,
        { data: { name: 'sharesubmit', cancel: true, content: 'Are you sure you want to Review the file?' }, width: '500px', panelClass: "deletemod",disableClose:true });
