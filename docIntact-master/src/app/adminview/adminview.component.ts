...
$(".ietop").css("margin-top","100px");
  ...
  getRecentFiles() {
    this.documentService.getSearch('documents/recentfiles').subscribe(data => {
      ...
      this.recentFileList.forEach(file => {
        ...
        this.documentsListDevices.push(file);
  ...
  Filelist(event) {
    ...
    if (event.isUserInput) {
      ...
      if (event.source.selected && !check && docname) {
        this.documentsIdList.push(docname._id);
      }
        ...
        for( var i = 0; i < this.documentsIdList.length; i++){
          if ( this.documentsIdList[i] === docname._id) {
           this.documentsIdList.splice(i, 1); 
            i--;
          }
        }
        }
  ...
  // this.StatusChart.data.labels = this.weekDates
  // this.StatusChart.data.datasets[0].data = this.weekdata;
  ...
  fileListDevices(event) {
    ...
    if (event.isUserInput) {
      ...
      if (event.source.selected && !check && docname) {
        ...
        this.documentsIdDevicesList.push(docname._id);
      }
      else if (check && !event.source.selected && docname) {

      for( var i = 0; i < this.documentsListDevices.length; i++){
        if ( this.documentsListDevices[i]._id === docname._id) {
         this.documentsListDevices.splice(i, 1); 
        
        }
      }
      this.documentsIdDevicesList.splice(this.documentsIdDevicesList.indexOf(docname._id), 1);
     }
      if (this.documentsIdDevicesList.length>=0) this.fileViewDevices();
  ...
  uniqueDeviceCount(list, result) {
    ...
    this.documentsListDevices.forEach(element => {
      ...
      browerCount.push({
        ...
        id: element._id,
        y: bCount.length
      });   /// inserting the values for browser
      andriodCount.push({
        name:element.name,
        id: element._id,
        y: aCount.length
  ...
  fileViewDevicesChart() {
    var v=[]
    var resultfile = []
    this.documentsListDevices = this.documentsListDevices.filter(file =>   file=file.name?file.name.split('.pdf' ):file.split('.pdf' ))
  ...
  fileViewTimings(event) {
    ...
    if (event.isUserInput) {
      ...
      if (event.source.selected && !check && docname) {
        this.documentsIdTimingList.push(docname._id);
        ...
        for( var i = 0; i < this.documentsIdTimingList.length; i++){
          ...
          for( var i = 0; i < this.recentFileList.length; i++){
            if ( this.recentFileList[i]._id === docname._id) {
             this.recentFileList.splice(i, 1); 
            }
            if ( this.documentsIdTimingList[i] === docname._id) {
              this.documentsIdTimingList.splice(i, 1); 
      ...
      if(this.documentsIdTimingList.length>=0) {
       if(!this.showyest1) this.FilelistTiming('today');
       if(this.showyest1) this.FilelistTiming('yesterday')}
    }
  }
